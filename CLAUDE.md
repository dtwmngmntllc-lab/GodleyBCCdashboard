...
## Known Issue — Email Archiver V2.4 GMAIL_BATCH_MODIFY_MESSAGES Tool Slug Drift (discovered Godley install, June 17 2026)

**Problem:** Email Archiver V2.4 calls Composio's `GMAIL_BATCH_MODIFY_MESSAGES` to mark emails as archived after processing. Composio returns `http=404 Tool not found` — the slug has either been renamed in the registry or was never accurate.

**Symptom:** Real-archive runs (records_processed > 0) flip to status `failed` with error message `GMAIL_BATCH_MODIFY_MESSAGES failed (http=404): Tool GMAIL_BATCH_MODIFY_MESSAGES not found`. Documents and attachments are written correctly; only the post-processing archive-marking step fails. The 0-emails path still succeeds because the call is skipped. Effect: Email Archiver looks healthy in success counts but actual archiving never completes.

**Fix:** Look up the correct Composio Gmail tool for batch label modification (likely `GMAIL_MODIFY_THREAD_LABELS` or similar) via `COMPOSIO_SEARCH_TOOLS`. Update the call site in `supabase/functions/email-archiver/index.ts`. Redeploy via `SUPABASE_DEPLOY_FUNCTION` (Composio) — not `SUPABASE_UPDATE_A_FUNCTION`, which only updates metadata. Re-set `verify_jwt=false` after deploy.

**Related backlog item:** V2.4 metadata structure (`{path, version, archive_query, messages_total}`) writes correctly on the 0-emails path but is empty `{}` on the real-archive path. Same call site needs the metadata structure replicated. Carries forward from S23 as a deferred item.

---

## Known Issue — Monthly Close Monitor Template-Creation Gap (discovered Godley install, June 17 2026)

**Problem:** The `monthly_close_monitor` plpgsql function implements only the overdue-alerting half of its documented behavior. The recipe description says "End-of-month creates next month's checklist by template" but no template-creation branch exists in the function body. Result: if `monthly_close_checklist` is empty, the recipe is a silent no-op forever — it loops over zero rows, logs `success` with `records_processed=0`, and never generates anything.

**Symptom:** No entries in `monthly_close_checklist`, no overdue alerts, but recipe shows `success` every run. The Monthly Close module on the dashboard is empty.

**Workaround used at Godley install:** Manually seeded the current and next periods (7 standard items each) directly into `monthly_close_checklist`. Standard SF agent monthly close template:

| Document | Category | Expected by |
|---|---|---|
| Bank Statement | financial | 7th of following month |
| Credit Card Statement | financial | 12th of following month |
| SF Daily Comp Recap (monthly) | compensation | 5th of following month |
| SF Deduction Statement | compensation | 10th of following month |
| Payroll Statements | payroll | 5th of following month |
| Producer Production Report | production | 5th of following month |
| GL Trial Balance Reconciled | reconciliation | 15th of following month |

**Permanent fix:** Add a date-based branch to the function: when `EXTRACT(DAY FROM CURRENT_DATE) >= 25` (last week of the month), insert the standard template rows for the next `period_year, period_month` if no rows exist for that period yet.

---

## Known Pattern — Four-Bucket Empty-Table Analysis (Godley install, June 17 2026)

**Context:** At end of S24, 29 of 38 agency-scoped tables were empty on the Godley install, causing most dashboard screens to show no data. Investigation in S25 sorted the empty tables into four functional buckets. This pattern applies to every BCC install — use it during post-install audits.

**Bucket 1 — agency-specific intake (9 tables):** `staff`, `positions`, `bank_accounts`, `credit_accounts`, `social_accounts`, `commission_structures`, `goals`, `persistent_memory`, `notification_preferences`. Require operator/owner input. Build into install handoff with a questionnaire.

**Bucket 2 — automation-fed (6 tables):** `comp_recap`, `credit_transactions`, `journal_entries`, `payroll_runs`, `producer_production`, `content_calendar`. Fill automatically once source documents arrive in Gmail/Drive and the relevant recipes run.

**Bucket 3 — derived/child tables (5):** `journal_lines`, `payroll_detail`, `aipp_tracking`, `scoreboard_tracking`, `monthly_close_checklist`. Fill alongside parent table writes (except `monthly_close_checklist` — see the template-creation gap above).

**Bucket 4 — event-driven, fill over time (9 tables):** `alerts`, `applicants`, `interviews`, `offers`, `onboarding_checklists`, `staff_performance`, `social_analytics`, `tasks`, `compliance_log`. Fill from events/automations as the business runs.

**Safe non-agency-specific seeds done at Godley S25:** `tasks` (setup-tracking tasks), `persistent_memory` (system-level operational facts), `monthly_close_checklist` (template entries for current and next period), `notification_preferences` (default alert categories), `onboarding_checklists` (template entries across producer/CSR/admin staff types). These same seeds are safe defaults for every future install — recommend baking into the seed migration.
