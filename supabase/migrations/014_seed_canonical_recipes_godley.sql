-- =============================================================================
-- Migration: 014_seed_canonical_recipes_godley
-- =============================================================================
-- Seeds the canonical 12 automation recipes for the Deatria Godley BCC install,
...
--
-- All recipes are agency-scoped to 0b8a0268-6b30-4e77-a40a-bc8ee77b5a6c.
-- 11 active; Social Media Scheduler inactive until FB/LinkedIn connections land.
-- =============================================================================

DO $$
DECLARE
    v_agency UUID := '0b8a0268-6b30-4e77-a40a-bc8ee77b5a6c';
    v_existing INT;
BEGIN
    ...
    INSERT INTO public.automation_recipes (
        ...
        'Monthly: parses producer monthly production reports (forwarded by Deatria), extracts issued premium per producer per line of business via Groq, writes to producer_production. Feeds the HR & People Performance tab.',
        'cron', '0 9 1 * *',
        'GMAIL_FETCH_EMAILS', 'gmail',
    ...
    INSERT INTO public.automation_recipes (
        ...
        'Composes morning briefing via Groq from real data — revenue YTD, AIPP, top tasks, alerts, today social posts. Sends to Deatria via Gmail.',
        'cron', '0 12 * * *',
        'GMAIL_SEND_EMAIL', 'gmail',
        'You are writing the morning briefing for Deatria. Tone: warm, direct, partner-not-assistant. Open with one sentence on what matters most today, then the standard sections (where we are, today''s priorities, compliance upcoming, what I''m watching, what to ask me).',
        jsonb_build_object(
            'recipient', 'dtwmngmntllc@gmail.com',
