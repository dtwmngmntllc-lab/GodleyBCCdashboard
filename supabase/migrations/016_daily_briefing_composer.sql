-- =========================================================================
-- MIGRATION 016 — Daily Briefing Composer (INTERNAL handler #4)
-- =========================================================================
-- The Daily Briefing Email recipe was shipped in migration 014 with a
-- hardcoded smoke-test body. AUTOMATIONS_INSTALL.md says it should compose
-- from real DB data — revenue YTD, AIPP, top tasks, alerts, today's posts.
--
-- This migration:
--   1. Adds a new INTERNAL handler `daily_briefing_composer` matching the
--      migration 012 pattern (SECURITY DEFINER, returns jsonb with
--      records_processed + output_summary).
--   2. Switches the Godley Daily Briefing recipe from
--      composio_action='GMAIL_SEND_EMAIL' to composio_action='INTERNAL' with
--      internal_handler='daily_briefing_composer'.
--
-- Why INTERNAL not Composio: the existing Composio path puts a single tool
-- call's input_config on the recipe row. Daily Briefing needs to (a) query
-- 5+ tables, (b) compose an HTML body, (c) THEN call GMAIL_SEND_EMAIL.
-- The runner has no compose-then-call orchestration. INTERNAL handlers can
-- do all three because they have access to the full SQL surface + pg_net
-- for outbound HTTP. Closes S6 Discovery 4.
-- =========================================================================
...
BEGIN
  -- ---------------------------------------------------------------------
  -- 1. Idempotency: don't double-send within a single day.
  -- ---------------------------------------------------------------------
  IF EXISTS (
  ...
  -- ---------------------------------------------------------------------
  -- 2. Recipe input_config (recipient is the one piece kept configurable)
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 3. Agency context for personalization
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 4. Composio credentials (Vault-aware via get_setting)
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 5. Financial aggregates (graceful on empty tables)
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 8. Today's scheduled content_calendar items
  -- ---------------------------------------------------------------------
  WITH today_posts AS (
...
$html$<div style="font-family:Helvetica,Arial,sans-serif;font-size:14px;color:#222;max-width:640px;line-height:1.5">
  ...
  <p style="color:#888;font-size:12px;margin-top:16px">
    ...
    <a href="https://godley-bc-cdashboard.vercel.app" style="color:#555">godley-bc-cdashboard.vercel.app</a>.
  ...
  -- ---------------------------------------------------------------------
  -- 10. Send via Composio GMAIL_SEND_EMAIL
  -- ---------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------
  -- 11. Wait briefly for the response, read status
  -- ---------------------------------------------------------------------
  PERFORM pg_sleep(6);
  ...
  IF NOT v_sent_ok THEN
    v_error_msg := 'http=' || COALESCE(v_status_code::text, 'pending') ||
                   ' body=' || COALESCE(LEFT(v_response, 300), '<no response yet>');
  END IF;

  -- ---------------------------------------------------------------------
  -- 12. Log to daily_briefing_log (snapshot the composed body for audit)
  -- ---------------------------------------------------------------------
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary',
      CASE
        WHEN v_sent_ok
          THEN 'Briefing sent to ' || v_recipient || ' (request_id=' || v_request_id || ')'
...
-- =========================================================================
-- Flip the Godley Daily Briefing recipe over to INTERNAL.
...
UPDATE public.automation_recipes
 ...
 WHERE agency_id = '0b8a0268-6b30-4e77-a40a-bc8ee77b5a6c'::uuid
   AND recipe_name  = 'Daily Briefing Email';
