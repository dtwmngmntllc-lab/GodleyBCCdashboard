...
//
// =========================================================================
// V2.6 V1-PATH METADATA PARITY (2026-06-17):
//   - V1 path now populates the same metadata jsonb shape as the V2 final
//     block, so monitoring queries against automation_run_log.metadata can
//     filter uniformly across both paths. V1-irrelevant dimensions
//     (attachments_uploaded, attachments_skipped_size,
//     messages_skipped_already_archived) are populated with zero/empty
//     defaults rather than omitted.
//   - V1 modify failures no longer throw to the outer catch block (where
//     they would land as `path: "unknown"` with just a crash message).
//     Instead they aggregate into modify_error + per_message_errors and
//     the run finalizes through the same writeOutcome path as a clean
//     success, with status=failed. Messages whose modify succeeded still
//     get a documents row; only the failed messages stay in the inbox
//     for the next run.
//   - V1 docs-insert-failure case (previously a separate early-return
//     branch) is now folded into the unified status calculation —
//     same metadata block, same return shape.
//   - Version markers in metadata writes bumped from V2.5 to V2.6 at all
//     five writeOutcome call sites.
//   - Behavior change: V2 path is unchanged. V1 path no longer 500s on
//     modify failures; it returns the same HTTP code as it would for a
//     docs-insert failure (500 when status=failed, 200 when success).
//
// =========================================================================
// LIMITS (V2.0):
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // =====================================================================
    // V1 path — flag off. V2.6: aggregate modify failures into structured
    // metadata rather than throwing to outer catch (which would log them
    // as path:unknown crash). Messages whose modify succeeded still get a
    // documents row; run is marked failed only if at least one failure
    // occurred. Metadata shape now mirrors the V2 final block exactly.
    // =====================================================================
    if (!routeAttachmentsToDrive) {
      ...
      // per message instead. V2.6: per-message failures are accumulated
      // (not thrown) so the run can finalize through writeOutcome with full
      ...
      for (const mid of messageIds) {
        ...
        if (modRes.ok) {
          archivedMessageIds.push(mid);
        } else {
          modifyFailures.push({ messageId: mid, error: `http=${modRes.httpStatus}: ${modRes.error}` });
        }
      }

      const modifyError: string | null = modifyFailures.length > 0
        ? `GMAIL_ADD_LABEL_TO_EMAIL failed for ${modifyFailures.length}/${messageIds.length} messages: ` +
          modifyFailures.slice(0, 3).map((f) => `${f.messageId}=${f.error}`).join("; ")
        : null;

      // Only log documents rows for messages whose modify succeeded; failed
      // messages remain in inbox for next-run retry.
      const archivedSet = new Set(archivedMessageIds);
      const now = new Date().toISOString();
      const docRows = messages
        .filter((m: any) => {
          const id = m?.messageId || m?.id;
          return typeof id === "string" && archivedSet.has(id);
        })
        .map((m: any) => {
      ...
      // V2.6: unified summary/status calculation matching V2 path shape.
      const summaryParts: string[] = [];
      summaryParts.push(`V1: ${archivedMessageIds.length}/${messageIds.length} messages archived`);
      if (modifyFailures.length > 0) summaryParts.push(`${modifyFailures.length} per-message errors`);
      summaryParts.push(`${docsInserted} documents rows written`);
      if (modifyError) summaryParts.push(`modify error: ${modifyError}`);
      ...
      // V2.6 metadata: mirrors V2 final-block shape. V1-irrelevant dimensions
      // (attachments_uploaded, attachments_skipped_size,
      // messages_skipped_already_archived) populated as zero/empty defaults
      // rather than omitted so monitoring queries can filter uniformly.
      const v1Metadata: Record<string, any> = {
        version: "V2.6",
        path: "V1",
        messages_total: messageIds.length,
        messages_archived: archivedMessageIds.length,
        messages_failed: modifyFailures.length,
        messages_skipped_already_archived: 0,
        ...
        per_message_errors: modifyFailures,
        archive_query: archiveQuery,
      };

      await writeOutcome(runStatus, archivedMessageIds.length, summary, errMsg, v1Metadata);
